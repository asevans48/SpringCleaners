...
class XMLTagCleaner {
  ...
  @BeanProperty
  var additionsToExistingTags: java.util.Map[String,java.util.Map[String,String]] = _ //creates properties on existing beans [xpath,[name,value or null for none (set lists and maps from attributes)]]
  ...
  def createTags(file: File): Future[Boolean] = Future {
    ...
    try{
      ...
      if(additionsToExistingTags != null){
         val xpath = XPathFactory.newInstance().newXPath()
         additionsToExistingTags.keySet.foreach{ x =>
           var nodes = xpath.compile(x).evaluate(doc,XPathConstants.NODESET).asInstanceOf[NodeList]
           if(nodes.getLength > 0){
             additionsToExistingTags.get(x).foreach{ kv =>
